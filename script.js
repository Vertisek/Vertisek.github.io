...
const DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/1544047297211625503/T_W92oJb9wzXqV7s0c-o1k-s7W6L1i-1i-1i-1i-1i-1i-1i'; // Wklej swój webhook Discorda, aby otrzymywać powiadomienia o nowych opiniach!
...
function initSupabase() {
    if (typeof window.supabase === 'undefined' || !window.supabase.createClient) {
        console.warn('Biblioteka Supabase nie jest załadowana.');
        return false;
    }
...
async function fetchReviewsFromSupabase() {
    ...
    try {
        ...
        console.error('Supabase fetch reviews exception:', e);
        return null;
    }
}
...
async function insertReviewToSupabase(review) {
    ...
    try {
        ...
        console.error('Supabase insert review exception:', e);
        return false;
    }
}
...
async function deleteReviewFromSupabase(id) {
    ...
    async function updateReviewVotesInSupabase(id, votesUp, votesDown) {
        ...
        try {
            const { error } = await supabase
                ...
                .eq('id', id);
            if (error) {
                console.error('Supabase update review votes error:', error);
                return false;
            }
            return true;
        } catch (e) {
            console.error('Supabase update review votes exception:', e);
            return false;
        }
    }

    async function uploadAudioToSupabase(file, reviewId, type) {
        if (!supabase || !file) return null;
        try {
            const ext = file.name.split('.').pop().toLowerCase();
            const path = `${reviewId}/${type}.${ext}`;
            ...
            const { data: urlData } = supabase.storage.from('review-audio').getPublicUrl(path);
    ...
    async function deleteAudioFromSupabase(reviewId) {
        if (!supabase) return;
        try {
            const paths = [`${reviewId}/before.mp3`, `${reviewId}/after.mp3`];
            await supabase.storage.from('review-audio').remove(paths);
        } catch (e) {
            console.error('Supabase storage delete error:', e);
        }
    }

    // --- Cloud Database Sync Engine (Firebase / REST API) — ZACHOWANE dla kompatybilności ---
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Inicjalizuj Supabase (biblioteka ładowana przez CDN przed tym skryptem)
        initSupabase();
    ...
    async function loadReviews() {
        // --- SUPABASE: pobierz opinie z bazy danych ---
        const supabaseReviews = await fetchReviewsFromSupabase();

        if (supabaseReviews !== null) {
            // Supabase jest skonfigurowane i odpowiedziało
            if (supabaseReviews.length === 0 && localStorage.getItem('vertone_reviews_initialized') !== 'true') {
                // Pierwsze uruchomienie — wgraj domyślne opinie do Supabase
                for (const rev of DEFAULT_REVIEWS) {
                    const supabaseRev = {
                        ...
                        rating: rev.rating,
                        text: rev.text,
                        avatar: rev.avatar || 'user1',
                        votes_up: rev.votesUp || 0,
                        votes_down: rev.votesDown || 0,
                        before_audio_url: null,
                        after_audio_url: null
                    };
                    await insertReviewToSupabase(supabaseRev);
                }
                localStorage.setItem('vertone_reviews_initialized', 'true');
                // Pobierz ponownie po wgraniu domyślnych
                const freshReviews = await fetchReviewsFromSupabase();
                appState.reviews = (freshReviews || []).map(mapSupabaseReview);
            } else {
                appState.reviews = supabaseReviews.map(mapSupabaseReview);
            }
            // Zapisz pobrane opinie do lokalnego IndexedDB dla trybu offline
            for (const rev of appState.reviews) {
                await saveToDB('reviews', rev);
            }
            return;
        }

        // --- FALLBACK: Supabase niedostępny — użyj IndexedDB ---
    ...
    // Mapuje rekord z Supabase (snake_case) na format używany przez appState
    function mapSupabaseReview(rev) {
        return {
            id: rev.id,
            author: rev.author,
            rating: rev.rating,
            ...
            votesDown: rev.votes_down || 0,
            beforeAudioUrl: rev.before_audio_url || null,
            afterAudioUrl: rev.after_audio_url || null,
    ...
    function renderReviews() {
        ...
        appState.reviews.forEach(rev => {
            ...
            // Check if there are before/after comparison files (Supabase URL lub legacy IndexedDB)
            let audioHTML = '';
            const hasBefore = rev.beforeAudioUrl || rev.beforeAudioData || rev.beforeAudioBlob;
            const hasAfter = rev.afterAudioUrl || rev.afterAudioData || rev.afterAudioBlob;
            ...
            document.querySelectorAll('.btn-delete-review').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    showCustomConfirm(t('confirm_delete_review') || "Czy na pewno usunąć tę opinię?", async () => {
                        // Usuń z Supabase (podstawowy backend)
                        const deletedFromSupabase = await deleteReviewFromSupabase(id);
                        if (deletedFromSupabase) {
                            // Usuń też pliki audio z Supabase Storage
                            await deleteAudioFromSupabase(id);
                        } else {
                            // Fallback: usuń z IndexedDB
    ...
    function handleReviewAudioPlayToggle(id, type) {
        ...
        // Supabase Storage URL (priorytet)
        const audioUrl = type === 'before' ? review.beforeAudioUrl : review.afterAudioUrl;
        // Legacy IndexedDB ArrayBuffer
        ...
        if (appState.currentPlayingId === playId) {
            ...
            if (audioUrl) {
                // Supabase Storage — bezpośredni URL publiczny
                audio.src = audioUrl;
            } else if (data) {
                // Legacy IndexedDB ArrayBuffer
    ...
    async function handleVote(type, id, voteType) {
        ...
        if (type === 'track') {
            ...
            if (item) {
                ...
                if (supabase) {
                    await updateReviewVotesInSupabase(id, item.votesUp, item.votesDown);
                }
                renderReviews();
            }
        }
    ...
    async function submitNewReview() {
        ...
        try {
            ...
            if (supabase) {
                // --- SUPABASE: zapisz opinię i wyślij pliki audio ---
                let beforeAudioUrl = null;
                let afterAudioUrl = null;

                // Upload plików audio do Supabase Storage
                if (beforeFile) {
                    if (submitBtn) submitBtn.textContent = '⏳ Wysyłam audio (przed)...';
                    beforeAudioUrl = await uploadAudioToSupabase(beforeFile, reviewId, 'before');
                }
                if (afterFile) {
                    if (submitBtn) submitBtn.textContent = '⏳ Wysyłam audio (po)...';
                    afterAudioUrl = await uploadAudioToSupabase(afterFile, reviewId, 'after');
                }

                if (submitBtn) submitBtn.textContent = '⏳ Zapisuję opinię...';

                // Rekord do Supabase (snake_case, bez danych binarnych)
                const supabaseReview = {
                    ...
                    votes_up: 0,
                    votes_down: 0,
                    before_audio_url: beforeAudioUrl,
                    after_audio_url: afterAudioUrl
                };

                const saved = await insertReviewToSupabase(supabaseReview);

                if (saved) {
                    // Powiadomienie Discord
                    sendDiscordWebhookNotification(
                        '⭐ Nowa Opinia na stronie Vertone!',
                        `**Autor:** ${author}\n**Ocena:** ${'★'.repeat(supabaseReview.rating)}\n**Treść opinii:** "${text}"`,
                        [
                            { name: 'Przed miksem Audio', value: beforeAudioUrl ? `✅ Tak` : 'Brak', inline: true },
                            { name: 'Po miksie Audio', value: afterAudioUrl ? `✅ Tak` : 'Brak', inline: true }
                        ]
                    );

                    // Zapisz kopię w lokalnej bazie IndexedDB
                    const localMapped = mapSupabaseReview(supabaseReview);
                    await saveToDB('reviews', localMapped);
                    ...
                    showCustomAlert("Błąd podczas zapisywania opinii. Spróbuj ponownie.");
                }
            } else {
                // --- FALLBACK: Supabase niedostępny — IndexedDB ---
            ...
            if (submitBtn) {
                ...
                submitBtn.textContent = 'Prześlij opinię';
            }
        }
    }
    ...
    saveApiKeyBtn.addEventListener('click', async () => {
        ...
        if (apiKey) {
            try {
                ...
                // Ustawienie nowej instancji w config
                if (typeof config !== 'undefined') {
                    config.API.API_KEY = apiKey;
                }
                alert('Klucz API został pomyślnie zapisany!');
            } catch (e) {
                console.error('Błąd zapisu klucza API:', e);
                alert('Wystąpił błąd podczas zapisywania klucza API!');
            }
        }
    });